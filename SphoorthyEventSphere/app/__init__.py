...
def create_app():
    ...
    def home():
        ...
        for e in ongoing + completed:
            ...
            poster_url = ""
            if e.get('poster'):
                # We need to find which folder this event is in
                club_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data', 'clubs', e['club_id'])
                if os.path.exists(club_dir):
                    for item in os.listdir(club_dir):
                        edir = os.path.join(club_dir, item)
                        if os.path.isdir(edir):
                            info_p = os.path.join(edir, 'info.json')
                            if os.path.exists(info_p):
                                with open(info_p) as f:
                                    try:
                                        if json.load(f).get('id') == e['id']:
                                            # Found the folder!
                                            poster_url = f"/static/uploads/clubs/{e['club_id']}/events/{item}/posters/{e['poster']}"
                                            break
                                    except: continue
        ...
        return render_template('index.html', user=user, clubs=clubs, ongoing=ongoing, completed=completed, active_elections=active_elections, college_events=college_events, inter_college_events=inter_college_events)
    ...
    def bulk_email_page(club_id):
        ...
        if not user: return redirect(url_for('login_page'))
        role = user.get('role', '')
        if role != 'super_admin' and role != f"{club_id}_admin":
            return "Unauthorized", 403
        club = DB.get_club_by_id(club_id)
        events = DB.get_events(club_id)
