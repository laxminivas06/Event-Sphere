...
# ─── Auth Helpers ─────────────────────────────────────────────────────────────
def is_manager(user):
    """Event Manager role — separate from super_admin."""
    return user and user.get('role') == 'event_manager'

def is_event_admin(user):
    """Event-specific admin created by the event manager."""
    return user and user.get('role') == 'event_admin'

def is_admin(user):
    """Any privileged user in the EM system: managers, admins, or club admins."""
    if not user: return False
    role = user.get('role', '')
    if role in ('event_manager', 'event_admin', 'super_admin'): return True
    if is_club_admin(user): return True
    return False
...
def admin_events(user):
    ...
    rec = next((a for a in em_admins if a.get('email') == identifier), None)
    if not rec: return []
    assigned = rec.get('assigned_events', [])
    return [e for e in all_ev if e['id'] in assigned]

def get_club_id_from_user(user):
    """Extract club_id from a club admin's role. e.g. 'creator_club_admin' -> 'creator_club'"""
    role = user.get('role', '')
    if role.endswith('_admin') and role != 'super_admin':
...
def is_club_admin(user):
    """Check if user is a club admin (any *_admin role except super_admin)."""
    if not user: return False
    role = user.get('role', '')
    return role.endswith('_admin') and role != 'super_admin'
...
def enrich_events_with_stats(events):
    """Attach ticket stats (_reg_count, _checked_in, _revenue, _cash_pending) to each event dict."""
    tickets = get_tickets()
    for ev in events:
        ...
        ev['_cash_pending']  = sum(1 for t in ev_tickets
...
<div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;background:#0f172a;color:#f1f5f9;border-radius:16px;overflow:hidden;">
  ...
  <div style="padding:2rem;">
    ...
    Mailer.send_email(email, subject, body, html, qr_path)
...
def em_dashboard():
    ...
    settings  = get_settings()
    em_admins = get_em_admins()
    all_events = get_events()  # for assignment dropdown
    return render_template('em_admin.html',
        user=user, events=events, all_tickets=tickets,
        settings=settings, em_admins=em_admins, all_events=all_events,
        clubs=clubs)
...
def _require_event_access(event_id):
    """Helper: returns (user, event, club_id) or redirects. Call as: result = _require_event_access(event_id); if isinstance(result, tuple): user, event, club_id = result; else: return result"""
    user = session.get('user')
    if not user:
        return redirect(url_for('login_page'))
    event, club_id = _club_event_access(user, event_id)
    if not event:
        return 'Event not found or access denied.', 403
...
def em_event_hub(event_id):
    ...
    return render_template('em_event_hub.html', user=user, event=event, club_id=club_id)
...
def api_qr_image(ticket_id):
    ...
    return send_file(_qr_buf(t['qr_data']), mimetype='image/png')
...
def api_assign_events(admin_id):
    ...
    for a in admins:
        if a['id'] == admin_id:
            a['assigned_events'] = data.get('event_ids', [])
            break
    save_em_admins(admins)
    return jsonify({'success': True})
...
def _can_access_hackathon(user, event_id):
    ...
    if role in ('event_manager', 'super_admin'): return event
    if is_evaluator(user):
        ev_list = evaluator_events(user)
        return event if any(e['id'] == event_id for e in ev_list) else None
    if is_event_admin(user):
        ev_list = admin_events(user)
        return event if any(e['id'] == event_id for e in ev_list) else None
    if is_club_admin(user):
...
def _can_access_techfest(user, event_id):
    ...
    if is_club_admin(user):
        club_id = get_club_id_from_user(user)
        if event.get('organized_by_id') == club_id: return event
        
    if is_event_admin(user):
        ev_list = admin_events(user)
        if any(e['id'] == event_id for e in ev_list): return event
...
def api_hackathon_register_team():
    ...
    for t in teams:
        all_member_ids = [m.get('roll_number') or m.get('email') for m in t.get('members', [])]
        if identifier in all_member_ids or t.get('leader_id') == identifier:
            return jsonify({'success': False, 'message': 'You or a member are already in a team for this hackathon'}), 400
...
def api_create_evaluator():
    ...
    event_id = data.get('event_id')
    assigned_events = [event_id] if event_id else []
